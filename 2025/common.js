...
var com = com||{};
const base_url='https://s.128877.xyz';
...
com.loadImages = function(stype){
	...
	//绘制棋盘
	com.bgImg = new Image();
	com.bgImg.src  = base_url+'/img/'+stype+'/bg.png';
	
	//提示点
	com.dotImg = new Image();
	com.dotImg.src  = base_url+'/img/'+stype+'/dot.png';
	
	//棋子
	for (var i in com.args){
		com[i] = {};
		com[i].img = new Image();
		com[i].img.src = base_url+'/img/'+stype+'/'+ com.args[i].img +'.png';
		//com[i].img.src = "img/"+stype+"/r_m.png";
	}
	
	//棋子外框
	com.paneImg = new Image();
	com.paneImg.src  = base_url+'/img/'+stype+'/r_box.png';
	
	document.getElementsByTagName('body')[0].style.background= 'url('+base_url+'/img/'+stype+'/bg.jpg)';
...
com.bylaw.m = function (x,y,map,my){
	var d=[];
		...
		// 	let key=play.map[y][x];
		// 	let cid=Object.keys(chessinfo).filter(n=>n.includes(key))[0];
		// 	let bk='div-'+(d[i][1]*9+d[i][0]);
		// 	let flag=checkBJP(cid,bk);
		// 	if(!!getblocks()[bk])flag=checkCJP(cid,bk);
		// 	console.log(cid,bk,flag);
		//  if(flag)d[i]=null;
		// }
		// d=d.filter(n=>!!n);
	return d;
...
com.args={
	...
	//蓝子
	'C':{text:'俥', img:'b_c', my:-1 ,bl:'c', value:com.value.C},
	'M':{text:'傌', img:'b_m', my:-1 ,bl:'m', value:com.value.M},
...
com.class.Man = function (key, x, y){
	...
	this.bl = function (map){
		...
		let arr=com.bylaw[o.bl](this.x,this.y,map,this.my);
		let cid=Object.keys(chessinfo).filter(n => n.includes(this.key))[0];
		return arr;
	};
};
